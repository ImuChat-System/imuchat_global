...
export default function FilesActivityScreen() {
    ...
    const grouped = useMemo(() => {
        ...
        for (const act of activities) {
            const dateKey = new Date(act.timestamp).toLocaleDateString();
    ...
    const renderActivity = (activity: typeof activities[0], isLast: boolean) => {
            ...
            <View key={activity.id} style={styles.item}>
                ...
                <View style={styles.content}>
                    <Text style={styles.action}>
                        ...
                        <Text style={styles.fileName}>{activity.file_name || activity.folder_name || ''}</Text>
                    </Text>
                    {activity.details && (
                ...
                <FlatList
                    ...
                    renderItem={({ item: [date, acts] }) => (
                        <View>
                            ...
                            {acts.map((act, idx) => renderActivity(act, idx === acts.length - 1))}
                        </View>
                    )}
