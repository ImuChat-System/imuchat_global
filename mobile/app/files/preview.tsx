...
import { formatFileSize, getFileColor, getFileIcon, isPreviewable } from '@/services/files-api';
...
export default function FilePreviewScreen() {
    ...
    const styles = useMemo(() => StyleSheet.create({
        ...
        headerTitle: { flex: 1, fontSize: 16, fontWeight: '600', color: colors.text, marginHorizontal: spacing.sm },
        previewArea: {
            flex: 1,
            alignItems: 'center',
        ...
        previewText: { fontSize: 14, color: colors.textSecondary, textAlign: 'center', marginTop: spacing.md },
        textContent: {
        ...
        versionNumber: { fontSize: 13, fontWeight: '600', color: colors.primary, width: 30 },
    ...
    if (!file) {
        return (
            <View style={styles.loading}>
                ...
                <Text style={{ color: colors.textSecondary, marginTop: spacing.md }}>{t('files.loading')}</Text>
            </View>
        );
    }
        ...
        <View style={styles.container}>
            ...
            <View style={styles.header}>
                ...
                <Pressable onPress={() => toggleFavorite(file.id)}>
                    <Ionicons name={file.is_favorite ? 'star' : 'star-outline'} size={22} color={file.is_favorite ? '#FFB300' : colors.textSecondary} />
                </Pressable>
            </View>
            ...
            <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
                ...
                <View style={styles.previewArea}>
                    {file.category === 'image' && file.local_uri ? (
                        <Image source={{ uri: file.local_uri }} style={styles.imagePreview} resizeMode="contain" />
                    ) : file.category === 'code' || file.extension === 'md' || file.extension === 'txt' ? (
                        <ScrollView style={styles.textContent}>
                            <Text style={styles.codeText}>{file.description || t('files.noPreview')}</Text>
                        </ScrollView>
                    ) : (
                        <>
                            <View style={[styles.iconPreview, { backgroundColor: getFileColor(file.category) + '20' }]}>
                                <Ionicons name={getFileIcon(file.category) as any} size={56} color={getFileColor(file.category)} />
                            </View>
                            <Text style={styles.previewText}>
                                {canPreview ? t('files.previewAvailableSoon') : t('files.noPreview')}
                            </Text>
                        </>
                    )}
                </View>
                ...
                <View style={styles.infoSection}>
                    ...
                    {file.tags.length > 0 && (
                        <>
                            <Text style={[styles.sectionTitle, { marginTop: spacing.md }]}>{t('files.tags')}</Text>
                    ...
                    {file.versions && file.versions.length > 0 && (
                        <>
                            <Text style={[styles.sectionTitle, { marginTop: spacing.md }]}>{t('files.versionHistory')}</Text>
                            {file.versions.map(ver => (
                                <View key={ver.id} style={styles.versionRow}>
                                    <Text style={styles.versionNumber}>v{ver.version_number}</Text>
                                    <Text style={styles.versionSummary}>{ver.change_summary}</Text>
                                    <Text style={styles.versionDate}>{new Date(ver.created_at).toLocaleDateString()}</Text>
                                </View>
                            ))}
                        </>
                    )}
            ...
            <View style={styles.actions}>
                <Pressable style={styles.actionBtn} onPress={() => router.push({ pathname: '/files/share', params: { id: file.id } })}>
                    <Ionicons name="share-outline" size={22} color={colors.primary} />
                    <Text style={styles.actionText}>{t('files.share')}</Text>
                </Pressable>
                <Pressable style={styles.actionBtn} onPress={() => showToast(t('files.downloadComingSoon'), 'info')}>
                    <Ionicons name="download-outline" size={22} color={colors.primary} />
                    <Text style={styles.actionText}>{t('files.download')}</Text>
                </Pressable>
                <Pressable style={styles.actionBtn} onPress={() => router.push('/files/activity')}>
                ...
                <Pressable style={styles.actionBtn} onPress={async () => {
                    ...
                    <Text style={[styles.actionText, { color: colors.error }]}>{t('files.delete')}</Text>
                </Pressable>
            </View>
        </View>
    );
