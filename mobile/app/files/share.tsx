...
export default function FileShareScreen() {
    ...
    const [selectedPermission, setSelectedPermission] = useState<typeof PERMISSIONS[number]>('view');
    ...
    const handleAddShare = useCallback(async () => {
        ...
        await shareFile(id, email.trim(), selectedPermission);
        setEmail('');
    ...
    const styles = useMemo(() => StyleSheet.create({
        ...
        linkToken: { fontSize: 12, fontFamily: 'monospace', color: colors.primary },
            ...
            <FlatList
                ...
                ListHeaderComponent={
                    <>
                            ...
                            <View style={styles.permRow}>
                                {PERMISSIONS.map(perm => {
                                    ...
                                    return (
                                        <Pressable
                                            key={perm}
                                            style={[styles.permChip, { backgroundColor: active ? col + '30' : colors.surface }]}
                                            onPress={() => setSelectedPermission(perm)}
                                        >
                                            <Ionicons name={PERMISSION_ICONS[perm] as any} size={14} color={active ? col : colors.textSecondary} />
                                            <Text style={[styles.permChipText, { color: active ? col : colors.textSecondary }]}>
                                                {t(`files.perm_${perm}`)}
                                            </Text>
                                        </Pressable>
                                    );
                        ...
                        {/* Current shares */}
                        <View style={styles.section}>
                            <Text style={styles.sectionTitle}>{t('files.currentShares')}</Text>
                            {shares.length > 0 ? (
                                shares.map(share => {
                                    ...
                                    return (
                                        <View key={share.id} style={styles.shareItem}>
                                            <View style={styles.shareInfo}>
                                                <Text style={styles.shareUser}>{share.shared_with_email || share.shared_with_id}</Text>
                                                <Text style={styles.sharePerm}>
                                                    {t('files.sharedBy')} {share.shared_by_name || ''}
                                                </Text>
                                            </View>
                                            ...
                                            <Pressable onPress={() => handleRemoveShare(share.id)} style={{ paddingLeft: spacing.sm }}>
                        ...
                        <View style={styles.section}>
                                ...
                                return (
                                    <View key={link.id} style={styles.linkItem}>
                                        <View style={styles.linkTopRow}>
                                            <Text style={styles.linkToken}>{link.token.substring(0, 16)}...</Text>
                                            <Text style={[styles.linkStatus, { color: isActive ? '#66BB6A' : colors.textSecondary }]}>
                                                {isActive ? t('files.linkActive') : t('files.linkInactive')}
                                            </Text>
                                        </View>
                                        <Text style={styles.linkMeta}>
                                            {t(`files.perm_${link.permission}`)}
                                            {link.expires_at ? ` · ${t('files.expiresAt')} ${new Date(link.expires_at).toLocaleDateString()}` : ''}
                                            {link.max_downloads ? ` · ${link.download_count}/${link.max_downloads} DL` : ''}
                                        </Text>
                                        <View style={styles.linkActions}>
                                            <Pressable style={styles.linkBtn} onPress={() => handleCopyLink(link.token)}>
                                                <Ionicons name="copy-outline" size={16} color={colors.primary} />
                                                <Text style={styles.linkBtnText}>{t('files.copyLink')}</Text>
                                            </Pressable>
                                            {isActive && (
                                                <Pressable style={styles.linkBtn} onPress={() => handleDeactivateLink(link.id)}>
                                                    <Ionicons name="close-circle-outline" size={16} color={colors.error} />
                                                    <Text style={[styles.linkBtnText, { color: colors.error }]}>{t('files.deactivate')}</Text>
                                                </Pressable>
                                            )}
                                        </View>
                                    </View>
                            ...
                            <Pressable
                                style={[styles.addBtn, { flexDirection: 'row', alignItems: 'center', gap: spacing.xs, alignSelf: 'flex-start', marginTop: spacing.sm }]}
                                onPress={handleCreateLink}
                            >
                                <Ionicons name="link" size={18} color="#FFF" />
                                <Text style={{ color: '#FFF', fontWeight: '600', fontSize: 13 }}>{t('files.createLink')}</Text>
                            </Pressable>
                        </View>
                    </>
                }
