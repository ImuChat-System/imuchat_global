...
export default function FilesTrashScreen() {
    ...
    const showTrashed = useFilesStore(s => s.showTrashed);
    const setShowTrashed = useFilesStore(s => s.setShowTrashed);
    ...
    const handleDeletePermanently = useCallback((fileId: string) => {
        Alert.alert(
    ...
    const styles = useMemo(() => StyleSheet.create({
        ...
        emptyTrashText: { fontSize: 13, fontWeight: '600', color: colors.error },
        info: {
            flexDirection: 'row',
            alignItems: 'center',
    ...
    const renderItem = ({ item }: { item: typeof trashedFiles[0] }) => {
        const trashedDate = item.trashed_at ? new Date(item.trashed_at).toLocaleDateString() : '';
        return (
            <View style={styles.item}>
                <View style={[styles.itemIcon, { backgroundColor: getFileColor(item.category) + '20' }]}>
                ...
                <View style={styles.itemActions}>
                    ...
                    <Pressable style={styles.actionBtn} onPress={() => handleDeletePermanently(item.id)}>
