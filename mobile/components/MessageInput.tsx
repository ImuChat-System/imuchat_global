...
/** Handle exposé par MessageInput pour un contrôle externe */
...
interface MessageInputProps {
  ...
  /** Called when text input changes (for bot command suggestions) */
...
const MessageInput = forwardRef<MessageInputHandle, MessageInputProps>(
  function MessageInput(
    ...
    // Expose setText / focus to parent via ref
    useImperativeHandle(
      ref,
      () => ({
        setText: (text: string) => {
    ...
    const canSend =
      (message.trim().length > 0 || selectedMedia) && !isUploading;
